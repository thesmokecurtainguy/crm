...
/** Where the import source tag is stored: a TEXT custom field on both entities. */
...
/** Header aliases we recognise automatically (Vibe Prospecting, HubSpot, our own sheets). */
...
function stripCredentials(name: string): string {
	// "Tom Donaghy, Aia, Ncarb, Leed Ap" -> "Tom Donaghy"; "(Chris) Christopher" kept simple.
	return (name.split(",")[0] ?? "")
		.replace(/\(.*?\)/g, "")
		.replace(/\s+/g, " ")
		.trim();
...
export function ImportForm() {
	...
	/** Find or create the row's company. Returns null for rows with no company at all. */
	async function resolveCompany(
		...
		if (companyId) {
			...
			try {
				...
				// Domain already taken by a record the search missed; look it up again.
	...
	/** Create the row's contact. Returns false when the row is company-only or a duplicate. */
				...
				<Field>
					...
					<FieldDescription>
						{table
							...
							: "Vibe Prospecting exports, HubSpot exports, and hand-built sheets all work. Headers are matched automatically; adjust below."}
					</FieldDescription>
				</Field>
